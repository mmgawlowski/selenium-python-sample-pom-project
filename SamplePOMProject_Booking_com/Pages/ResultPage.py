...
class ResultPage(BasePage):
    ...
    def check_result_place_or_destination(self, query):
        result = self.driver.find_element_by_xpath(self.search_destination_result_xpath)
        if query in result.text:
            return True
        else:
            try:
                self.driver.find_element_by_xpath(self.search_result_xpath % query).is_displayed
            except NoSuchElementException:
                return False
        result = self.driver.find_element_by_xpath(self.search_result_xpath % query)
        if query in result.text:
            return True
        return False
    ...
    def validate_check_in_date(self, check_in):
        return self.driver.find_element_by_xpath(self.search_result_check_in_date_xpath % check_in).is_displayed

    # Same as above for check out date.
    def validate_check_out_date(self, check_out):
        return self.driver.find_element_by_xpath(self.search_result_check_out_date_xpath % check_out).is_displayed
